...
pub async fn get_place_to_eat() -> Result<impl warp::Reply, std::convert::Infallible> {
    let offset = 0;
    Ok(warp::reply::json(&choose_place_to_eat(offset)))
}

pub async fn get_place_to_eat_with_offset(
    offset: u32,
) -> Result<impl warp::Reply, std::convert::Infallible> {
    Ok(warp::reply::json(&choose_place_to_eat(offset)))
}

pub async fn get_place_to_eat_in_the_next_days(
    days: u32,
) -> Result<impl warp::Reply, std::convert::Infallible> {
    let next_days: Vec<PlaceToEatDay> = (0..days)
        .into_iter()
...
fn choose_place_to_eat(offset: u32) -> PlaceToEat {
    ...
    let chosen_list = places[distribution.sample(&mut get_rnd(&offset, 1).0)]
        .0
        .clone();
...
fn get_days_since_my_birth() -> u32 {
    ...
    now.signed_duration_since(birthday).num_days() as u32
}
